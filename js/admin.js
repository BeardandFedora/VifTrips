 ...
 *************************/
 ...
 /**
  * Init the Trip Infos Metabox
  */
 function gpInitMboxTripInfos() {
 	var mboxTripInfos = jQuery( '#mbox_trip_infos' );

 	if ( mboxTripInfos.length > 0 ) {

 		/* Click on "Choose a file" */

		mboxTripInfos.on( 'click', '.mbox-trip-infos-choose-file', function(e){
			e.preventDefault();

			var chooseFileLink = jQuery( this );
			var currentTD = chooseFileLink.parents( 'td' );
			var deleteFileLink = currentTD.find( '.mbox-trip-infos-delete-file' );
			var fileField = currentTD.find( 'input[name=vif-file]' );
			var filePreview = currentTD.find( '.mbox-trip-infos-file-preview' );
		...
		mboxTripInfos.on( 'click', '.mbox-trip-infos-delete-file', function(e){
			e.preventDefault();

			mboxTripInfos.find( '.mbox-trip-infos-choose-file' ).show();
			jQuery( this ).hide();
			mboxTripInfos.find( 'input[name=vif-file]' ).val('');
			mboxTripInfos.find( '.mbox-trip-infos-file-preview' ).empty();
...
function gpInitMboxMapPreview() {
	...
	if ( mboxMapPreview.length > 0 ) {
		mboxMapPreview.find( '.vif-leaflet-map' ).gpLeafletMap();
	}
...
function gpInitMboxMarkerInfos() {
	...
	if ( mboxMarkerInfos.length > 0 ) {
		...
		var inputMarkerIconType = mboxMarkerInfos.find( 'input[name=vif-icon-type]' );
		var inputMarkerIconFilename = mboxMarkerInfos.find( 'input[name=vif-icon-filename]' );
		var inputMarkerContentOrder = mboxMarkerInfos.find( 'input[name=vif-popup-content-order]' );
		...
		mboxMarkerInfos.on( 'change', 'input[name=vif-marker-icons-list]:radio', function(e){
			...
			if ( !listTypeRequested.is( ':visible' ) ) {
				...
				if ( jQuery( this ).val() == 'custom' ) {
					...
					if ( listTypeRequested.find( 'ul' ).length == 0 ) {
						...
						jQuery.ajax({
							url: ajaxurl,
							data: {action: 'vif-get-markers-icons-list', list_type: 'custom' },
		...
		mboxMarkerContentEditImageWrap.on( 'click', '.mbox-marker-content-edit-image-choose', function(e){
			...
			var imageField = mboxMarkerContentEditImageWrap.find( 'input[name=vif-popup-image]' );
			...
			imageMediaManagerFrame.on( 'select', function(){
				...
				jQuery.ajax({
					url: ajaxurl,
					data: { action: 'vif-get-mbox-image-preview', id: imageID },
		...
		mboxMarkerContentEditImageWrap.on( 'click', '.mbox-marker-content-edit-image-delete', function(e){
			...
			jQuery( this ).hide();
			mboxMarkerContentEditImageWrap.find( 'input[name=vif-popup-image]' ).val('');
		...
		mboxMarkerContentEditVideoWrap.on( 'change paste keyup', '#mbox-marker-content-edit-video-url', function(e){
			...
			curInput.data( 'timer', setTimeout( function() {
				...
				if ( curInput.data( 'url' ) != curInput.val() ) {
					...
					jQuery.ajax({
						url: ajaxurl,
						data: { action: 'vif-get-video-player', url: curInput.val() },
						success: function( html ) {
							if ( html != '' ) {
		...
		mboxMarkerContentEditAudioWrap.on( 'click', '.mbox-marker-content-edit-audio-choose', function(e){
			...
			var audioField = mboxMarkerContentEditAudioWrap.find( 'input[name=vif-popup-audio]' );
			...
			audioMediaManagerFrame.on( 'select', function(){
				...
				jQuery.ajax({
					url: ajaxurl,
					data: { action: 'vif-get-mbox-audio-preview', id: audioID },
					success: function( html ) {
		...
		mboxMarkerContentEditAudioWrap.on( 'click', '.mbox-marker-content-edit-audio-delete', function(e){
			...
			jQuery( this ).hide();
			mboxMarkerContentEditAudioWrap.find( 'input[name=vif-popup-audio]' ).val('');
...
function gpInitSettingsPage() {
	var settingsMap = jQuery( '.vif-leaflet-map-container' );

	if ( settingsMap.length > 0 ) {

        // Init Map
		settingsMap.find( '.vif-leaflet-map' ).gpLeafletMap();

        // Init ColorPickers
        var titleColPick = jQuery( '#setting-title-color' ).wpColorPicker();
        var taglineColPick = jQuery( '#setting-tagline-color' ).wpColorPicker();
        var primaryColPick = jQuery( '#setting-primary-color' ).wpColorPicker();
        var secondaryColPick = jQuery( '#setting-secondary-color' ).wpColorPicker();
	}
