/**
 * VifTrips Plugin wrapper for leaflet
...
;(function ( $, window, document, undefined ) {
	...
	function Plugin( element, options ) {
		...
		this.jqMapWrapper = this.jqMap.parent( '.vif-leaflet-map-wrap' );
		this.jqMapContainer = this.jqMapWrapper.parent( '.vif-leaflet-map-container' );
	...
	Plugin.prototype.loadMapMarkers = function() {
		...
		if ( this._mapID != 0 ) {
			...
			var ajaxAction = ( this._mapID == 'all' ) ? 'vif-get-all-markers' : 'vif-get-map-markers';
	...
	Plugin.prototype.createMarker = function( markerInfos, withPopup ) {
		...
		if ( typeof markerInfos === 'object' ) {
			...
			if ( withPopup ) {
				...
				markerContent += '<p class="map-popup-marker-more vif-clearfix"><a href="' + markerInfos.pmlnk + '"' + ( ( this._isEmbed ) ? ' target="_parent"' : '' ) + '>' + gpGlobalI18n.i18nMarkerMore + '</a></p>';
	...
	Plugin.prototype.changeDragMarkerIcon = function() {
		jQuery.ajax({
			url: gpGlobalVars.ajaxurl,
			context: this,
			data: { action: 'vif-get-all-icon-infos', icontype: this._dragMarkerIconType, iconfilename: this._dragMarkerIconFilename },
			success: function( resp ) {
	...
	Plugin.prototype.addMarkersIndex = function() {
		...
		if ( this._hasMarkersIndex && this._markersJSON != '' ) {
			...
			var _this = this;
			var markersIndexContainer = jQuery( '<div class="vif-map-markers-index vif-map-action-pan">' );
	   		var markersIndexUL = jQuery( '<ul class="vif-clearfix">' );
    		...
    		for ( var markerIndex in this._markersJSON ) {
				if ( this._markersJSON.hasOwnProperty( markerIndex ) ) {
					...
					markersIndexUL.append(
						jQuery( '<li class="vif-map-marker" data-marker-id="' + this._markersJSON[markerIndex].id + '">' )
							.append(
								jQuery( '<a href="" class="vif-clearfix" title="' + gpGlobalI18n.i18nMarkersIndexClickMarker + '">' )
									.append(
										jQuery( '<p class="vif-map-marker-icon">' )
											.append( '<img src="' + this._markersJSON[markerIndex].icon.icon_url + '" alt="' + gpGlobalI18n.i18nMarkerIcon + '">' )
									)
									.append(
										jQuery( '<p class="vif-map-marker-title">' )
			...
			if ( markersIndexUL.find( 'li' ).length > 0 ) {
				...
				var mapActionsToggles = this.jqMapContainer.find( '.vif-map-actions-toggles' );
				var markersIndexToggle = jQuery( '<a href="" class="vif-map-markers-index-toggle" title="' + gpGlobalI18n.i18nMarkersIndexToggle + '">' ).html( gpGlobalI18n.i18nMarkersIndexToggle );

				if ( mapActionsToggles.length > 0 ) {
					mapActionsToggles.prepend( markersIndexToggle );
				}
				else {
					mapActionsToggles = jQuery( '<div class="vif-map-actions-toggles vif-clearfix">' );
					mapActionsToggles.prepend( markersIndexToggle );
				...
				mapActionsToggles.on( 'click', '.vif-map-markers-index-toggle', function(e){
					e.preventDefault();

					var toggleButton = jQuery( this );
					var currentOpenedPan = _this.jqMapContainer.find( '.vif-map-action-pan.active' );
					var mapIndexPan = _this.jqMapContainer.find( '.vif-map-markers-index' );
				...
				markersIndexContainer.on( 'click', '.vif-map-marker a', function(e){
					e.preventDefault();
	...
	Plugin.prototype.addExportMap = function() {
		...
		// Create the actions toggles container if necessary
		var mapActionsToggles = this.jqMapContainer.find( '.vif-map-actions-toggles' );
		var exportMapToggle = jQuery( '<a href="" class="vif-map-export-map-toggle" title="' + gpGlobalI18n.i18nExportMapToggle + '">' ).html( gpGlobalI18n.i18nExportMapToggle );
		...
		// Create Export Pan
		var exportPan = jQuery( '<div class="vif-map-export vif-map-action-pan vif-clearfix">' )
			.append( '<h3>' + gpGlobalI18n.i18nExportMapTitle + '</h3>' )
			.append(
				jQuery( '<p class="vif-map-export-height">' )
					.append( '<label>' + gpGlobalI18n.i18nExportMapHeight + '</label>' )
					.append( '<input type="text" id="vif-map-export-height" name="vif-map-export-height" value="">' )
			)
			.append(
				jQuery( '<p class="vif-map-export-code">' )
					.append( '<label>' + gpGlobalI18n.i18nExportMapCopyPaste + '</label>' )
					.append( '<textarea id="vif-map-export-code" readonly="readonly" name="vif-map-export-code" cols="10" rows="3"></textarea>' )
			);

		this.jqMapContainer.append( exportPan );

		// Click on Export Map Toggle
		mapActionsToggles.on( 'click', '.vif-map-export-map-toggle', function(e){
			e.preventDefault();

			var toggleButton = jQuery( this );
			var currentOpenedPan = _this.jqMapContainer.find( '.vif-map-action-pan.active' );
			var mapExportPan = _this.jqMapContainer.find( '.vif-map-export' );
		...
		// Export fields
		var mapHeightField = this.jqMapContainer.find( '#vif-map-export-height' );
		var mapExportCodeField = this.jqMapContainer.find( '#vif-map-export-code' );
	...
	Plugin.prototype.showEmbedMapPermalink = function() {
		...
		if ( this._isEmbed && this._mapTitle != '' && this._mapOriginalUrl != '' ) {
			...
			var mapLink = jQuery( '<a href="' + this._mapOriginalUrl + '" id="vif-map-original-page" target="_parent">&larr; ' + this._mapTitle + '</a>' );
