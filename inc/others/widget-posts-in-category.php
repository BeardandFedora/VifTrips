...
class vif_widget_posts_in_category extends WP_Widget {

	function __construct() {
		parent::__construct(
			'vif_wdg_posts_in_category', // Base ID
			__( 'Recents Posts in a category', 'lang_viftrips' ), // Widget name that appears in UI
			array(
				'description' => __( 'Display the titles of the recents posts in the choosen category', 'lang_viftrips' )
			) // Widget description 
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$last_posts = vif_query_get_posts_in_category( $category_term->term_id, array( 'posts_per_page' => $instance['nb_posts'] ) );
		...
		if ( count( $last_posts ) > 0 ) : ?>
			...
			<ul>
				...
				<?php foreach ( $last_posts as $p ) : ?>
					...
					<li>
						<a href="<?php echo get_permalink( $p ); ?>" title="<?php _e( 'Click to see this post', 'lang_viftrips' ); ?>">
			...
			<p class="widget-posts-in-list-more clearfix">
				<a href="<?php echo get_term_link( $category_term, 'category' ) ?>" title="<?php _e( 'See all', 'lang_viftrips' ); ?>">
					<?php _e( 'See all', 'lang_viftrips' ); ?>
				</a>
			</p>
	...
	public function form( $instance ) {
		...
		if ( isset( $instance['category'] ) ) {
			...
			$category = 1;
			$nb_posts = VIF_WIDGET_POSTS_IN_CATEGORY_NB_POSTS;
		}
		...
		<p class="widget-posts-in-category">
			...
			<select id="<?php echo $this->get_field_id( 'nb_posts' ); ?>" name="<?php echo $this->get_field_name( 'nb_posts' ); ?>">
				...
				<?php for ( $i = 1; $i <= VIF_WIDGET_POSTS_IN_CATEGORY_NB_POSTS;  $i++ ) : ?>
